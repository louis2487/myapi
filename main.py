...
# -------------------- Kakao / 운행일지(recode) --------------------
# 서버의 운행일지(recode) + Kakao 헬퍼 + runtime update API는 유지보수 최적화를 위해
# `recode/` 패키지로 분리되었습니다.
# - recode/db.py: ensure_recode_columns
# - recode/kakao.py: kakao helpers
# - recode/router.py: /recode/*, /runtime/*, /kakao/driving-info
...
# runtime/recode 관련 모델/엔드포인트는 `recode/router.py`로 분리됨.
...
# 운행일지(recode) + runtime update API는 유지보수 최적화를 위해 `recode/router.py`로 분리되었습니다.
# (RecodeService.kt가 호출하는 /recode/*, /runtime/update, /kakao/driving-info 포함)
